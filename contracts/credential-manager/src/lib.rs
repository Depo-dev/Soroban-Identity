...
const MAX_ISSUERS_CFG: Symbol = symbol_short!("ISSCAP");
...
pub enum ContractError {
    ...
    InvalidMaxIssuers = 14,
    InvalidSchemaHash = 15,
}
...
impl CredentialManager {
    ...
    pub fn add_issuer(env: Env, issuer: Address) -> Result<(), ContractError> {
        ...
        if !issuers.contains(&issuer) {
            if issuers.len() >= Self::get_max_issuers_internal(&env) { return Err(ContractError::MaxIssuersReached); }
